...
def add_material(payload: MaterialCreate, db: Session = Depends(get_db)):
    if not payload.material_code.strip() or not payload.material_name.strip():
        return {"error": "material_code and material_name are required"}
...
def material_by_code(material_code: str, db: Session = Depends(get_db)):
    x = find_material(db, material_code=material_code)
    if x:
