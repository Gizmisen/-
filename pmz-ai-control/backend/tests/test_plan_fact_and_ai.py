...
from app.services.ai_service import answer_query, get_ai_capabilities
...
def test_ai_can_build_context_document_local():
    Session = make_db()
    with Session() as db:
        answer, data, mode = answer_query(db, "Собери сводный документ", advanced=False)
        assert "Собрал документ" in answer
...
def test_ai_build_document_by_specific_query():
    Session = make_db()
    with Session() as db:
        answer, data, mode = answer_query(db, "Собери документ по просрочкам", advanced=False)
        assert "Собрал документ" in answer
        assert isinstance(data, dict)
        assert "overdue_orders" in data.get("sections", {})
        assert mode == "local"


def test_ai_capabilities_structure():
    caps = get_ai_capabilities()
    assert "advanced_available" in caps
    assert "default_mode" in caps
    assert "tools" in caps
    assert "build_document_by_query" in caps["tools"]
